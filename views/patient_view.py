...
class ProfilePage(QtWidgets.QWidget):
    def __init__(self, patient_id, user_context=None):
        ...
        subtitle = QtWidgets.QLabel(
            "Hoàn thiện thông tin để nhân viên tiếp nhận nhanh khi bạn đến khám."
        )
        subtitle.setWordWrap(True)
        subtitle.setStyleSheet("color: #64748b; font-size: 13px; margin-bottom: 6px;")
        layout.addWidget(subtitle)

        form_widget = QtWidgets.QWidget()
        form_widget.setObjectName("patientProfileCard")
        form_widget.setStyleSheet(
            "QWidget#patientProfileCard { background: white; border-radius: 12px; border: 1px solid #eef0f2; }"
            "QWidget#patientProfileCard QLabel { background: transparent; border: none; padding: 0; color: #1e293b; font-size: 14px; }"
        )
        ...
        self.dob_input.setDate(QtCore.QDate(2000, 1, 1))
        ...
        self.email_input.setPlaceholderText("Ví dụ: nguyenvana@example.com")
        ...
        self.address_input.setPlaceholderText("Nhập địa chỉ liên hệ")
        self.intake_notes_input = QtWidgets.QPlainTextEdit()
        self.intake_notes_input.setPlaceholderText("Ghi chú hỗ trợ tiếp nhận (nếu có)")
        self.intake_notes_input.setFixedHeight(72)
        ...
        form_layout.addRow("CCCD/CMND *:", self.cccd_input)
        form_layout.addRow("Email *:", self.email_input)
        form_layout.addRow("Nghề nghiệp *:", self.occupation_input)
        form_layout.addRow("Địa chỉ *:", self.address_input)
        form_layout.addRow("Ghi chú tiếp nhận:", self.intake_notes_input)
    ...
    def load_data(self):
        ...
        if p:
            ...
            self.phone_input.setText(str(p.get("phone") or ""))
            self.cccd_input.setText(str(p.get("cccd") or ""))
            self.email_input.setText(str(p.get("email") or ""))
            self.occupation_input.setText(str(p.get("occupation") or ""))
            self.address_input.setText(str(p.get("address") or ""))
            self.intake_notes_input.setPlainText(str(p.get("intake_notes") or ""))
            self.gender_input.setCurrentText(str(p.get("gender") or "Nam"))
            if p.get("dob"):
                dob = QtCore.QDate.fromString(str(p.get("dob")), "yyyy-MM-dd")
                if dob.isValid():
    ...
    def save_data(self):
        ...
        if missing:
            QtWidgets.QMessageBox.warning(self, "Thất bại", f"Thiếu thông tin bắt buộc ({' / '.join(missing)}).")
            return
        if not re.fullmatch(r"0\d{9}", phone):
            QtWidgets.QMessageBox.warning(self, "Thất bại", "SĐT không hợp lệ. Yêu cầu 10 chữ số và bắt đầu bằng 0.")
            return
        if not re.fullmatch(r"\d{12}", cccd):
            QtWidgets.QMessageBox.warning(self, "Thất bại", "CCCD/CMND không hợp lệ. Yêu cầu đúng 12 chữ số.")
            return
        if not re.fullmatch(r"[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}", email):
            QtWidgets.QMessageBox.warning(self, "Thất bại", "Email không đúng định dạng.")
            return
        ...
        payload.update(
            {
                ...
                "address": address,
                "intake_notes": self.intake_notes_input.toPlainText().strip(),
            }
        )
